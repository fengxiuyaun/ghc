...
tcMatchesFun fn@(L _ fun_name) matches exp_ty
  ...
  where
    ...
    match_ctxt = MC { mc_what = FunRhs fn False, mc_body = tcBody }
    ...
    tc_match ctxt pat_tys rhs_ty match@(Match _ pats maybe_rhs_sig grhss)
      = add_match_ctxt match $
        do { (pats', grhss') <- tcPats (mc_what ctxt) pats pat_tys $
           ...
           ; return (Match (tcHsMatchContext $ mc_what ctxt) pats' Nothing grhss') }
           -- ; return (Match CaseAlt pats' Nothing grhss') } -- ++AZ++ temporary CaseAlt for testing
